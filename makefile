...
CC_OPTIONS = /GS- /c /W3 /DWIN32_LEAN_AND_MEAN /GR- /EHsc /D _NO_CRT_STDIO_INLINE /D _CTYPE_DISABLE_MACROS 
...
# Compiler version 15 and below do not support option /FS (sync write of PDB)
!IF [cl_version_LE.bat 15]
CC_OPTIONS =  $(CC_OPTIONS)
!ELSE
CC_OPTIONS =  $(CC_OPTIONS) /FS
!ENDIF
...
# giving the lib a name that corresponds to MS compiler version can be good idea
# 
PROJ = minicrt15
...
$O\$(PROJ).LIB: {$O}$(OBJS)
    LIB /OUT:$O\$(PROJ).LIB $(OBJS) /NODEFAULTLIB
