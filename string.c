...
/* in VS2019 __ascii_isdigit && __ascii_isalpha are macros, that are being
   replaced with __chvalidchk function, doing the actual check. 
   This causes a compiler error, because the function definition will be
   changed into an invalid expression. To make this compilable the option
   _CTYPE_DISABLE_MACROS must be used. */
...
long __cdecl strnicmp(char const *compareString1, char const *compareString2, size_t count)
{
    char c1, c2;
    const char *p1,*p2;
    int d;
    ...
    do
    {
        c1 = (TCHAR)CharLower((TCHAR*)(*p1++));
        c2 = (TCHAR)CharLower((TCHAR*)(*p2++));
        d = c1 - c2;
        count--;
    }
    while (count > 0 && d == 0 && c1 != 0 && c2 != 0);

    return d;
}

long __cdecl stricmp(char const *compareString1, char const *compareString2)
{
    const char *p1,*p2;
    ...
    return (int) tolower((unsigned char) *p1) -
    (int) tolower((unsigned char) *p2);
}

char * __cdecl strdup(char const *src) {
